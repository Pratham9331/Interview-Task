...
namespace SearchOrtPagingTask.Controllers
{
    public class EmployeeController : Controller
    {
            ...
            int page=1, int pageSize=3)
        {
            ...
            var employeePerPage = sortedEmployees.ToPagedList(page, pageSize);
            EmployeeModel employee = new EmployeeModel()
            {
                Employees = employeePerPage,
                CurrentPage = page,
                TotalPages = employeePerPage.PageCount
            };
        ...
        private List<EmployeeViewModel> SearchedEmployees(string search)
        {
            ...
            {
                employees = _db.Employees.Select(e =>
                new EmployeeViewModel()
                {
                    EmployeeId = e.EmployeeId,
                    Name = e.Name,
                ...
                switch (sortColumn)
                {
                    ...
                    case "DepartmentId":
                        if(ViewBag.SortOrderDepartmentId == "asc"){
                            sortedEmployees.Sort((x, y) => x.DepartmentId.CompareTo(y.DepartmentId));
